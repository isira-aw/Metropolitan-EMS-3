#!/bin/bash
base="/home/claude/generator-frontend-complete"

# Admin Dashboard
cat > "$base/app/admin/dashboard/page.tsx" << 'EOF'
'use client';
...
export default function AdminDashboard() {
  const [stats, setStats] = useState({ users: 0, generators: 0, tickets: 0 });
  ...
  const loadStats = async () => {
    try {
      ...
      setStats(prev => ({ ...prev, users: usersRes.data.totalElements }));
    } catch (error) {
    ...
    <div className="space-y-6">
        ...
        <div className="card bg-green-500 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-4xl font-bold">{stats.generators}</p>
            </div>
      ...
      <div className="card">
        ...
        <div className="space-y-2">
          <a href="/admin/users" className="block p-4 bg-blue-50 hover:bg-blue-100 rounded">
            <p className="font-semibold">Manage Users</p>
            <p className="text-sm text-gray-600">Create, edit, and manage system users</p>
          </a>
          <a href="/admin/generators" className="block p-4 bg-green-50 hover:bg-green-100 rounded">
            <p className="font-semibold">Manage Generators</p>
            <p className="text-sm text-gray-600">Add and manage generator assets</p>
          </a>
          <a href="/admin/tickets" className="block p-4 bg-purple-50 hover:bg-purple-100 rounded">
            <p className="font-semibold">Create Ticket</p>
            <p className="text-sm text-gray-600">Create maintenance tickets and assign employees</p>
          </a>
        </div>
      </div>
    </div>
